...
use shared::packet::{ClientboundPacket, Packet, PacketHandler};
...
impl User {
    pub async fn connect(username: String, id: UserId, mut stream: TcpStream) -> io::Result<User> {
        ...
        stream.send_packet(Packet::Clientbound(ClientboundPacket::Token { token: token.clone() })).await?;

        Ok(User { username, id, token, stream })
    }

    pub async fn kick(&mut self, reason: &str) -> io::Result<()> {
        self.stream.send_packet(Packet::Clientbound(ClientboundPacket::Kicked { reason: reason.to_string() })).await
    }
